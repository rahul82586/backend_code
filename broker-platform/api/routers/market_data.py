...
async def get_tick_history(
    ...
    tick_repo: IHistoricalTickRepository = Depends(get_historical_tick_repository),
):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Failed to fetch tick history: {e}"
        )
...
async def get_bar_history(
    ...
    bar_repo: IHistoricalBarRepository = Depends(get_historical_bar_repository),
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch bar history: {e}"
        )
